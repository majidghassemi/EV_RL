...
class DeepQLearning:
    def __init__(self, adjacency_matrix, num_nodes, charging_stations, gamma=0.9, epsilon=0.2, alpha=0.1,
                 epsilon_decay_rate=0.98, min_epsilon=0.01, battery_charge=80, replay_buffer_size=10000,
                 batch_size=64, target_update_freq=10, learning_rate=1e-3):
    ...
    def select_action(self, state, battery_charge):
        current_node = state
        battery_charge_tensor = torch.tensor([battery_charge / self.initial_battery_charge], device=self.device, dtype=torch.float32)
        ...
        if random.random() < self.epsilon:
            ...
            with torch.no_grad():
                current_node_tensor = torch.tensor([current_node], device=self.device, dtype=torch.long)
                q_values = self.policy_net(current_node_tensor, battery_charge_tensor)
                # Create a mask of valid actions
                mask = torch.full((self.num_nodes,), float('-inf'), device=self.device)
    ...
    def train_step(self):
        ...
        battery_charge_batch = torch.tensor(batch[5], device=self.device, dtype=torch.float32) / self.initial_battery_charge
    ...
    def train(self, start_state, end_state, num_epochs, visualize=True, save_video=True):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            while not done:
                ...
                reward, next_battery_charge = self.reward_function(state, action_item, battery_charge)
                epoch_reward += reward
                ...
                # Store transition
                self.store_transition(state, action, reward, action_item, done, battery_charge, next_battery_charge)
            ...
            print(f"Epoch {epoch}: Total Reward: {epoch_reward}, Epsilon: {self.epsilon}")

            if visualize:
                filename = f"epochs/deepq_epoch_{epoch}.png"
                self.plot_graph(src_node=path[0],
