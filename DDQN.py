...
class DoubleDeepQLearning:
    ...
    def select_action(self, state, battery_charge):
        ...
        if random.random() < self.epsilon:
            ...
            with torch.no_grad():
                ...
                action = masked_q_values.argmax(dim=1, keepdim=True)
        return action
    ...
    def train_step(self):
        ...
        state_batch = torch.tensor(
            batch[0], device=self.device, dtype=torch.long
        )
        action_batch = torch.tensor(
            batch[1], device=self.device, dtype=torch.long
        ).unsqueeze(1)
        reward_batch = torch.tensor(
            batch[2], device=self.device, dtype=torch.float32
        )
        next_state_batch = torch.tensor(
            batch[3], device=self.device, dtype=torch.long
        )
        done_batch = torch.tensor(
            batch[4], device=self.device, dtype=torch.float32
        )
        ...
        # Compute next Q-values using policy network for action selection
        with torch.no_grad():
            next_q_values_policy = self.policy_net(
                next_state_batch, next_battery_charge_batch
            )
            # Mask invalid actions and select best actions
            max_actions = []
            for idx, next_state in enumerate(next_state_batch):
                ...
                if len(valid_actions) == 0:
                    max_actions.append(torch.tensor([0], device=self.device))
                else:
                    mask = torch.full(
                        (self.num_nodes,), float("-inf"), device=self.device
                    )
                    mask[valid_actions] = 0.0
                    masked_q_values = next_q_values_policy[idx] + mask
                    max_action = masked_q_values.argmax().unsqueeze(0)
                    max_actions.append(max_action)
            max_actions = torch.stack(max_actions)

            # Compute next Q-values using target network for action evaluation
            next_q_values_target = self.target_net(
                next_state_batch, next_battery_charge_batch
            )
    ...
    def reward_function(self, s_cur, s_next, battery_charge):
        ...
        battery_consumed = (
            self.adjacency_matrix[int(s_cur)][int(s_next)] * 0.85
        )
    ...
    def train(
        self, start_state, end_state, num_epochs, visualize=True, save_video=True
    ):
        print("-" * 20)
        print("Double Deep Q-Learning begins ...")
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            while not done:
                ...
                reward, next_battery_charge = self.reward_function(
                    state, action_item, battery_charge
                )
                epoch_reward += reward

                # Check if episode is done
                if action_item == end_state or next_battery_charge <= 0:
                    done = True
                else:
                    done = False
            ...
            # Decay epsilon
            self.epsilon = max(
                self.min_epsilon, self.epsilon * self.epsilon_decay_rate
            )
            ...
            if visualize:
                filename = f"epochs/doubledq_epoch_{epoch}.png"
                self.plot_graph(
                    src_node=path[0],
                    added_edges=list(zip(path[:-1], path[1:])),
                    figure_title=f"Double DQN: Epoch {epoch}, Reward: {epoch_reward}",
                    filename=filename,
                )
        ...
        print(
            f"Best battery charge: {self.best_epoch_results['battery']}"
        )
        print(
            f"Best reward: {self.best_epoch_results['reward']}"
        )
        print(
            f"Minimized Travel Time: {self.best_epoch_results['travel_time']}"
        )
        ...
        if visualize and save_video:
            ...
            imageio.mimsave("double_dqn.gif", images, fps=5)
